...
_DATA_CACHE = None
SAMPLES_PER_CLASS = [1, 2, 3, 4, 5, 6, 10, 11, 20, 21,  25, 40, 41, 100, 400]
...
class DataSets:
    def __init__(self, name, train_labeled: DataSet, train_unlabeled: DataSet, test: DataSet, valid: DataSet, train_original: DataSet,
                 height=32, width=32, colors=3, nclass=10, mean=0, std=1, p_labeled=None, p_unlabeled=None, p_original=None):
        self.name = name
    ...
    def creator(cls, name, seed, label, valid, augment, parse_fn=record_parse, do_memoize=False,
        ...
        def create():
            p_labeled = p_unlabeled = p_original = None
            ...
            train_original = DataSet.from_files(
                [DATA_DIR +'/' + name + '-train.tfrecord'], augment[0], parse_fn, image_shape)
            if do_memoize:
            ...
            return cls(name + '.' + FLAGS.augment + fullname + '-' + str(valid)
                       ...
                       train_original=train_original.skip(valid),
...
def create_datasets(augment_fn):
    ...
    d.update([DataSets.creator('cifar10', seed, label, valid, augment_fn)
              for seed, label, valid in itertools.product(range(10), [10 * x for x in SAMPLES_PER_CLASS], [1, 5000])])
    d.update([DataSets.creator('cifar10p', seed, label, valid, augment_fn)
              for seed, label, valid in itertools.product(range(16), [10 * x for x in SAMPLES_PER_CLASS], [1, 5000])])
    ...
    d.update([DataSets.creator('svhn_noextra', seed, label, valid, augment_fn)
              for seed, label, valid in itertools.product(range(6), [10 * x for x in SAMPLES_PER_CLASS], [1, 5000])])
    d.update([DataSets.creator('svhnp', seed, label, valid, augment_fn)
              for seed, label, valid in itertools.product(range(8), [10 * x for x in SAMPLES_PER_CLASS], [1, 5000])])
    d.update([DataSets.creator('svhnp_noextra', seed, label, valid, augment_fn)
              for seed, label, valid in itertools.product(range(8), [10 * x for x in SAMPLES_PER_CLASS], [1, 5000])])
    return d
